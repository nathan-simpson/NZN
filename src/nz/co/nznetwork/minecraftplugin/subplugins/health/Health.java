...
public class Health implements NZNSubplugin, CommandExecutor, CountdownListener {
    ...
    private boolean heal(CommandSender sender, String[] args) {
        if (!(sender instanceof Player) && args.length == 0) {//check they are a player if using on self
            sender.sendMessage(plugin.getUtils().ERROR_COLOUR + "Only players can use /heal on themselves");
            return true;
        }
        if (args.length != 0 && !plugin.getUtils().getPermision(sender, "nzn.health.heal.target")) { //check they have permission
            sender.sendMessage(plugin.getUtils().ERROR_COLOUR + "Incorrect format, use: /heal");
            return true;
        }
        if (args.length > 1) {//check the format of command
            sender.sendMessage(plugin.getUtils().ERROR_COLOUR + "Incorrect format, use: /heal <target>");
            return true;
        }
        Player player;
        if (args.length == 0) {
    ...
    private boolean god(CommandSender sender, String[] args) {
        if (!(sender instanceof Player) && args.length == 0) {//check they are a player if using on self
            sender.sendMessage(plugin.getUtils().ERROR_COLOUR + "Only players can use /god on themselves");
            return true;
        }
        if (args.length != 0 && !plugin.getUtils().getPermision(sender, "nzn.health.god.target")) { //check they have permission
            sender.sendMessage(plugin.getUtils().ERROR_COLOUR + "Incorrect format, use: /god");
            return true;
        }
        if (args.length > 1) {//check the format of command
            sender.sendMessage(plugin.getUtils().ERROR_COLOUR + "Incorrect format, use: /god <target>");
            return true;
        }
