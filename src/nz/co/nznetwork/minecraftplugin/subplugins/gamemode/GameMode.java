...
public class GameMode implements NZNSubplugin, CommandExecutor {
	...
	@Override
	public void reloadConfig() {} //dosn't use its own config

	@Override
	public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
		if(cmd.getName().equalsIgnoreCase("gm")) {
			if(!(sender instanceof Player)) {
				sender.sendMessage(plugin.getUtils().ERROR_COLOR+"Only players may use this command");
				return true;
			}
			...
			if(args.length == 0) { //tried to use gm on self with no permission
				sender.sendMessage(plugin.getUtils().ERROR_COLOR+"You do not have permission to use this command");
				return true;
			}
			...
			if(args.length == 1) {//tried to use gm on another player with no permission
				if(plugin.getUtils().getPermision(p, "nzn.gm.self")) {//however can do gm on self
					sender.sendMessage(plugin.getUtils().ERROR_COLOR+"You do not have permission to use this command on others, use /gm");
					return true;
				}
				sender.sendMessage(plugin.getUtils().ERROR_COLOR+"You do not have permission to use this command");
				return true;
			...
			if(args.length > 1) {//had too many arguments
				if(plugin.getUtils().getPermision(p, "nzn.gm.self")) {
					if(plugin.getUtils().getPermision(p, "nzn.gm.target")) {//has perms to use on others
						sender.sendMessage(plugin.getUtils().ERROR_COLOR+"Too many arguments, use /gm <player>");
						return true;
					}
					sender.sendMessage(plugin.getUtils().ERROR_COLOR+"You do not have permission to use this command on others, use /gm");
					return true;
